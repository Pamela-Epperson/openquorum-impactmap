...
import { STATE_META } from "./states.config";
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}} onClick={onClose}>
      <div style={{maxWidth:460,width:"100%"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={onClose} style={{padding:"9px 16px",borderRadius:8,border:"1px solid #555",background:"transparent",color:"#aaa",cursor:"pointer",fontSize:13}}>Close</button>
        </div>
      ...
      <div style={{background:"#fff",borderRadius:12,padding:"1.5rem",maxWidth:640,width:"100%",maxHeight:"80vh",display:"flex",flexDirection:"column",boxShadow:"0 8px 40px rgba(0,0,0,0.2)"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          <div>
            ...
            <p style={{margin:0,fontSize:11,color:"#888"}}>Generated by OpenQuorum · ready to send to journalists</p>
          </div>
      ...
      <div style={{padding:"0.9rem 1rem",cursor:"pointer"}} onClick={onToggle}>
        <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
          <div style={{flex:1}}>
            ...
            <p style={{margin:0,fontSize:12,color:"#888"}}>Serves: <strong style={{color:"#555",fontWeight:500}}>{board.populationLabel}</strong></p>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
            <p style={{margin:"0 0 1px",fontSize:20,fontWeight:600,color:vs.border}}>{impact.deferred.toLocaleString()}</p>
            <p style={{margin:"0 0 1px",fontSize:10,color:"#aaa"}}>deferred decisions</p>
            <p style={{margin:0,fontSize:12,color:"#777",fontWeight:500}}>{impact.months}mo · {impact.rate}% vacant</p>
          </div>
        </div>
      </div>
        ...
        <div style={{padding:"0 1rem 1rem",borderTop:"1px solid #f5f5f5"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(100px,1fr))",gap:8,margin:"0.85rem 0"}}>
            {[
              ...
              <div key={s.l} style={{background:"#f8f8f7",borderRadius:8,padding:"0.6rem 0.75rem"}}>
                <p style={{margin:"0 0 2px",fontSize:10,color:"#aaa",textTransform:"uppercase",letterSpacing:"0.05em"}}>{s.l}</p>
                <p style={{margin:"0 0 1px",fontSize:16,fontWeight:600,color:"#1a1a1a"}}>{s.v}</p>
                <p style={{margin:0,fontSize:10,color:"#aaa"}}>{s.s}</p>
              </div>
            ))}
          </div>
          ...
          {statement&&(
            <div style={{background:"#f8f8f7",borderRadius:8,padding:"0.85rem 1rem",marginBottom:10,borderLeft:"3px solid #1D9E75"}}>
              <p style={{margin:"0 0 5px",fontSize:10,fontWeight:500,color:"#aaa",textTransform:"uppercase",letterSpacing:"0.07em"}}>Impact statement — press & testimony ready</p>
          ...
          <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
            ...
            <a href={sv.applyUrl} target="_blank" rel="noreferrer"
              style={{padding:"6px 14px",borderRadius:8,border:"1px solid #eee",background:"transparent",color:"#888",fontSize:12,textDecoration:"none"}}>
              {sv.applyLabel ? `Apply via ${sv.applyLabel.split("—")[0].trim()} →` : "Apply to this seat →"}
            </a>
          </div>
        </div>
      )}
...
export default function ImpactMap() {
  ...
  // Derives state list from STATE_META — auto-updates as new states are added to states.config.js
  const stateList = useMemo(() => Object.keys(STATE_META), []);
  ...
  return(
    <div style={{fontFamily:"system-ui,-apple-system,sans-serif",maxWidth:960,margin:"0 auto",padding:"0 0 3rem",color:"#1a1a1a"}} onClick={()=>showMenu&&setShowMenu(false)}>
      ...
      <div style={{borderBottom:"1px solid #eee",paddingBottom:"1rem",marginBottom:"1.25rem"}}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
          <div>
            ...
            <p style={{margin:0,fontSize:12,color:"#888"}}>Constituent harm calculator — translating vacancy duration into real human cost</p>
          </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10,marginBottom:"1.25rem"}}>
        {[
          ...
          <div key={s.l} style={{background:"#f8f8f7",borderRadius:8,padding:"0.85rem 1rem"}}>
            <p style={{margin:"0 0 3px",fontSize:11,color:"#888",textTransform:"uppercase",letterSpacing:"0.06em"}}>{s.l}</p>
            <p style={{margin:"0 0 2px",fontSize:20,fontWeight:600,color:s.c,letterSpacing:"-0.02em"}}>{s.v}</p>
            <p style={{margin:0,fontSize:11,color:"#aaa"}}>{s.s}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{background:"#f8f8f7",borderRadius:8,padding:"1rem 1.25rem",marginBottom:"1.25rem"}}>
        <p style={{margin:"0 0 12px",fontSize:11,fontWeight:500,color:"#888",textTransform:"uppercase",letterSpacing:"0.07em"}}>Deferred decisions by domain — click to filter</p>
        {domainBars.map(({domain:d,deferred,pct})=>{
          ...
          return(
            <div key={d} style={{display:"flex",alignItems:"center",gap:10,marginBottom:7,cursor:"pointer"}} onClick={()=>setDomain(domain===d?"all":d)}>
              ...
              <span style={{fontSize:11,color:"#888",minWidth:90}}>{deferred.toLocaleString()} deferred</span>
            </div>
          );
        })}
      </div>
      ...
      {/* Footer */}
      <div style={{marginTop:"1.5rem",paddingTop:"1rem",borderTop:"1px solid #f0f0f0",fontSize:11,color:"#bbb",lineHeight:1.7}}>
